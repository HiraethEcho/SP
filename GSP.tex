...
The purpose of this article is to show that two different Mori fibre spaces as output of a klt pair can be linked by composition of  Sarkisov links.
...
On the other hand, to obtaining Sarkisov links, we need to run a special MMP known as $2$-ray game. More precisely, there are two cases:
\begin{enumerate}
  \item Find a contraction $g:X \to T$ such that $\rho(X/T)=2$ and factor though $f:X \to S$, then we run MMP on  $X $ over $T$, and obtains a Sarkisov link of type III or IV;
    \item  Find a divisorial contraction $p:Z\to X$, and therefore $\rho(Z/S)=2$. Then we run MMP on $Z$ over $S$, and obtains a Sarkisov link of type I or II.
\end{enumerate}
...
Let $f:X\dashrightarrow Y$ be a birational map of normal quasi-projective varieties, and $p:W\to X$ and $q:W\to Y$ be a resolution of indeterminacy of $f$, and $D$ be a $\mathbb{R}$-divisor divisor on $X$ with $f_*D=D_Y$. Then $f$ is called \textbf{$D$-non-positive ($D$-negative)} if
\begin{itemize}
...
\begin{enumerate}[A]
  ...
  \item  If we have a link $X_{i} \to X_{i+1}$ as case \ref{a4}, then next link is still the case \ref{a}. When running MMP on $(X_{i+1},B_{i+1}+\frac{1}{\mu_{i+1}}H_{i+1})$ wih scaling. we chose the same director  $C$ as in the former link.
...
First we need to show the procesure constructed in the last subsection terminates in finitely many steps. To prove the termination of Sarkisov program, we need following the discreteness of nef threshold  $\mu$:
\begin{cor}\label{Discreteness}
  The nef threshold $ \mu $ with respect to $ \theta $ is discrete.
\end{cor}
\begin{proof}
  Notice that all pairs in $ \mathcal{C}_\theta $ are klt and $ \delta $-lc, thus form a bounded family. Take the integral $ I $ in  lemma \ref{effeciveindex}, then $ I(K_X+B) $ is cartier. Take a rational curve $ C $ in $ \overline{\operatorname{NE}}(X/S) $, then by (JK, cor 2.8.4)
...
We prove it by contraction. Otherwise, if there is an infinite sequence, i.e. there are infinitely many $ X_i $ and birational maps obtained from the program:
...
\begin{proof}[proof of \ref{nfi}]
  \begin{enumerate}
    \item Only need to show $ (K_X+B+\frac{1}{\mu'}H) $ is $ f $-nef.  Suppose  $ B=\sum_ib_iB_i+\sum_jd_jD_j $ and $ B'=\sum_jd_j'D_j+\sum_kb_k'B_k' $, where $ B_i $ are divisors on $ X $ but not on $ X' $, $ B_k' $ are divisors on $ X' $ but not on $ X $, and $ D_j $ are divisors on both $ X $ and $ X' $. Let $W$ be the common resolution:
      \[ \xymatrix{
      &(W,B_W,H_W)\ar[dl]_{\sigma}\ar[dr]^{\sigma'}&\\
          (X,B,H)\ar[d]_f\ar@{.>}[rr]^\Phi&&(X',B',H')\ar[d]^{f'}\\
      S&&S'} \]
      Consider the ramification formulas:
      \[
        \begin{aligned}  
          K_W+B_W+\frac{1}{\mu'}H_W=&\sigma'^*(K_{X'}+B'+\frac{1}{\mu'}H')+\sum e'_jE_j+ \sum \sigma_k'G_k'\\
          =&\sigma^*(K_{X}+B+\frac{1}{\mu'}H)+\sum \sigma_iG_i+\sum e_jE_j
        \end{aligned}
      \]
      Here $ \{G_i\}, \{E_j\} $ are $ \sigma $-exceptional divisors, and $ \{E_j\}\{G'_k\} $ are $ \sigma' $-exceptional divisors. Since $H_W:=\sigma'^*H' $, $ g_k'>0 $ (or there are no such $ G'_k $). Then take a general curve $ C\subset X $ contracted by $ f $,  avoiding $ G_i, E_j $, and not contained in $ G'_k $, and identifies its strict transform $ \tilde{C} $ in $ W $ with $ C $. By computing intersection numbers:
      \[
        \begin{aligned}
          C.(K_X+B+\frac{1}{\mu'}H)=&\tilde{C}.\sigma^*(K_X+B+\frac{1}{\mu'}H)
          \\=&C.((K_X+B+\frac{1}{\mu'}H)+\sum g_iG_i+\sum e_jE_j)\\
          =&C.(K_W+B_W+\frac{1}{\mu'}H_W)\\
          =&C.(\sigma'^*(K_{X'}+B'+\frac{1}{\mu'}H')+\sum e'_jE_j+ \sum g_k'G_k')\\
          =&C.\sigma'^*f'^*A'+C.(\sum g_k'G_k')\\
          \geqslant&0
        \end{aligned}
      \]
      Since $ A' $ is ample on $ S' $, and $ g'_k>0 $. This shows $ (K_X+B+\frac{1}{\mu'}H) $ is $ f $-nef and $ \mu\geqslant \mu' $;
    \item  First we show that $ \mu=\mu' $. By 1), dually only need to show $ (K_{X'}+B'+\frac{1}{\mu}H') $ is $ f' $-nef. Take a curve $ C' $ similar as above, then
      \[
        \begin{aligned}
          C'.(K_{X'}+B'+\frac{1}{\mu}H')=&C'.(\sigma^*(K_{X}+B+\frac{1}{\mu}H)+\sum e_jE_j+ \sum g_k'G'_k)\\
          =&C'.(\sigma^*(K_{X}+B+\frac{1}{\mu}H)+C.(\sum g_k'G_k')\\
          \geqslant&0
        \end{aligned}
      \]
Thus $ (K_{X'}+B'+\frac{1}{\mu}H')$ is $f'$-nef.  Take a very ample divisor $ D $ on $ X $ and let $ D'  $ be strict transform on $ X' $, $ D' $ is $ f' $-ample, thus for some $ 0<d\ll1 $ 
      \begin{itemize}
        \item $ K_X+B_X+\frac{1}{\mu }H_X+dD $ is ample ;
...
\subsection{Prepare}
Let $(W,B_W)$ be a klt pair and $f:(X,B)\to S$ and $f':(X',B')\to S'$ be two different log Mori fibre spaces obtained by running $(K_{W}+B_{W})$-MMP. Take  very general ample $\mathbb{Q}$-divisors $ A $ and $ A' $ on $ S $ and $ S' $ such that $ G\sim_{\mathbb{Q}}-(K_X+B)+f^*A $ and $ H\sim_{\mathbb{Q}}-(K_{X'}+B')+f^{'*}A' $ are ample, and find $ G_W $ and $ H_W $ on $ W $ such that $ p^*G=p^{-1}_*G=G_W $ and $ q^*H=q^{-1}_*H=H_W $. To modify the beginning setting, we need more conventions and lemmas:
\begin{lem}
...
\end{itemize}
\end{lem}
By  above lemma, we replace $(W,B_W)$ by $(W',B_{W'})$, then $(W,B_{W})$ is terminal and $p:W\to X$ and $q:W\to X'$ are $(K_W+B_W)$-non-positive morphisms, and $(W,B_W)\geqslant (X,B),(X',B')$. Furthermore, we may assume $(W,B_W+gG_W+hH_W)$ is log smooth and terminal for all $0\leqslant g,h\leqslant 2$ by taking furthermore blowing up if necessary. Then we have Sarkisov program with double scaling of $(G_{W},H_{W})$: 
...
% TODO: induction: assume $X_{i}$, then construct $X_{i+1}$. 

This subsection we construct the links inductively. Suppose 
% Suppose $C_{i}$ is the $f_{i}$-vertical curve on $X_{i}$, then denote $r_{i}:=\frac{H_{i}.C_{i}}{G_{i}.C_{i}}$ and $D_{i}=B_{i}+g_{i}G_{i}+h_{i}H_{i}$.  
\begin{itemize}
...
% In case (A), we have $(W,D_{W}(s+\epsilon))\geqslant (Y,D_{Y}+\epsilon)$.
