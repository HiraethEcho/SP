...
% Let $H$ be the birational transform of $H'$ on $X$, then we run $(K_{X}+B+cH)$-MMP for maximum  $c$ such that
...
%Moreover, let $\mathcal{C}_{i}$ be the closure of $\mathcal{A}_{i}$ in $\mathcal{L}_A(V)$. If a divisor $D$ is contained in the boundary  $\partial \mathcal{E}_{A}(V)$ and three or more $\mathcal{C}_{i}$, then those morphisms form a Sarkisov link (Theorem \ref{constructlink}).
